...
window.onload= function(){
	...
	function startAd() {
		...
		var cta1				= document.getElementById("frame5_ctaback1");
		var cta2				= document.getElementById("frame5_ctaback2");
		var learn				= document.getElementById("frame5_lmbtn");
		...
		function startAnimation(){
			...
			introTimeline.to(swimmer, 4.5, {scaleX: 0.7, scaleY: 0.7, rotation:0.001, alpha:1, ease: Power2.easeOut, force3D:false});
	 		...
	 		introTimeline.from(text1c, 0.3, {scale:5.50, rotation: -5, x: 50, y:0, alpha:0, ease: Power2.easeOut},"-=3.5");
			...
			introTimeline.to(frame_3, 1.5, {x: -80, y:-58, scale: 0.65, alpha: 1, ease: Elastic.easeOut.config(0.8, 0.6)},"+=0.5");

			//takeaway copy
			introTimeline.to(text2a, 0.3, {x:100, y:-20, opacity:1, ease: Power2.easeOut},"-=0.9");
			introTimeline.to(text2b, 0.3, {x:100, y:-20, opacity:1, ease: Power2.easeOut},"-=0.7");
			introTimeline.to(text2c, 0.3, {x:100, y:-20, opacity:1, ease: Power2.easeOut},"-=0.5");

	 		//product image
	 		introTimeline.from(product, 0.7, {rotation: 150, x:250, y:100, alpha: 0, ease: Back.easeInOut.config(1.3), force3D:false},"-=1.5");
	 		
	 		//CTA
	 		introTimeline.from(frame5_animate, 0.4, {x:-250, y:55, alpha: 0, ease: Power2.easeOut, onComplete:addEventListeners},"+=0.1");		
		...
		function handleOver(event){
			...
			TweenLite.to(product, 0.5, {x:0, y:-15, ease: Power2.easeOut, force3D:false, overwrite:true});
			TweenLite.to(cta2, 0.3, {opacity:1, ease: Power2.easeOut,overwrite:true});
			TweenLite.to(learn, 0.5, {scale: 1.1, rotation: 360, ease: Power2.easeOut,overwrite:true});
		}
		function handleOut(event){
			TweenLite.to(h2hglow, 0.5, {opacity:0, ease: Power2.easeOut,overwrite:true});
		 	TweenLite.to(product, 0.5, {x:0, y:-2, ease: Power2.easeOut, force3D:false, overwrite:true});
		 	TweenLite.to(cta2, 0.3, {opacity:0, ease: Power2.easeOut,overwrite:true});
		 	TweenLite.to(learn, 0.5, {scale: 1, rotation: -360, ease: Power2.easeOut,overwrite:true});
		}
