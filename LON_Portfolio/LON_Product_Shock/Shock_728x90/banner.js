...
window.onload= function(){
	...
	function startAd() {
		...
		function startAnimation(){
			...
			introTimeline.to(frame_3, 1.5, {x: -208, alpha: 1, ease: Elastic.easeOut.config(0.8, 0.7)},"+=0.5");
			...
			introTimeline.to(text2d, 0.3, {x:100, y:-20, opacity:1, ease: Power2.easeOut},"-=0.6");
	 		...
	 		//CTA
	 		introTimeline.from(cta, 1, {x:-100, y:0, alpha: 0, ease: Elastic.easeOut.config(0.8, 0.8), onComplete:addEventListeners},"-=0.2");	
		...
		function handleOver(event){
			...
			TweenLite.to(product, 0.5, {y:-20, ease: Power2.easeOut, force3D:false, overwrite:true});
		}
		function handleOut(event){
			TweenLite.to(h2hglow, 0.5, {opacity:0, ease: Power2.easeOut,overwrite:true});
		 	TweenLite.to(product, 0.5, {y:0, ease: Power2.easeOut, force3D:false, overwrite:true});
		}
